...
- Raw-ingestion hardening is now accepted; `scripts/capture_site.py` exposes a safe operator contract via `--help` and `--print-contract`, rejects tracked output paths outside `snapshots/raw`, and will write richer capture metadata on future refreshed snapshots.
...
- Browser Use/node runtime is still below the version required for the in-app browser path, but that no longer blocks the mainline ingestion path because Python Playwright capture is already working locally.
...
- Current active phase: publication polish and first upstream push on top of the accepted local bundle.
- Current honest next step: execute the user-requested README overhaul and repository publish wave.
- Why that step is next: the current local bundle and mission-runtime corrections are already accepted, so the remaining work is repo-facing presentation and upstream publication rather than another mandatory runtime fix.
...
- Local Browser Use/node runtime is still below the version required to start the in-app browser workflow, but that is now a secondary environment branch rather than the mainline blocker.

## Deferred Hardening

- The raw capture runner is contract-hardened, but a fresh authenticated refresh has not yet materialized the new `manifest.captureContract`, `manifest.routeCapture`, and per-record `capture.*` metadata into newly generated raw artifacts because no auth env vars are currently set in this shell.
- That deferred refresh proof matters for future data-refresh maintenance, not for the current usable local-data agent surface.
